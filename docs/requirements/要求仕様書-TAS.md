...
[タイマー](#61-タイマー機能)
[アラーム](#62-アラーム機能)
[ストップウォッチ](#63-ストップウォッチ機能)
各々の機能に切り替えて実行できる。

1. **タイマー機能**はユーザーが設定した時間でカウントダウンを実行する。また、指定時間に達した際、ユーザーへ通知する

2. **アラーム機能**は5つまでアラームを記憶でき、24時間表記で、時間と分の単位でアラームが設定できる

3. **ストップウォッチ機能**は経過時間の計測(開始・一時停止・再開)とラップタイムの記録ができる
...
- ユーザーが時間を気にすることなく、集中して作業が出来る

- ユーザーが画面を見続ける負担を無くし、時間管理をサポートする

#### 2. アラーム

- ユーザーの時間配分を正確に管理するアラーム機能を提供する

- ユーザーのタスクにおける集中度や推移率の向上を図る

#### 3. ストップウォッチ

- ユーザーが複数のタイム記録を蓄積・管理し効率よく時間を管理する
...
| 機能        | 目的      |
| ---------- | --------- |
| 時間設定機能 | ユーザーがタイマーでカウントダウンしたい時間を、直感的かつ誤りなく設定できるようにする                                         |
| 開始機能     | ユーザーが設定した時間を基に、意図したタイミングですぐにカウントダウンを開始し残り時間を正確に把握できるようにする                 |
| 一時停止機能 | カウントダウンの途中でユーザーが必要に応じてタイマーを一時停止し、残り時間を保持したまま後から再開できるようにする　　　　　         |
| リセット機能 | カウントダウンの途中または終了後であっても、ユーザーがタイマーの状態を初期化し、開始前に設定した時間に戻して再利用できるようにする    |
| 終了通知機能 | タイマーの設定時間が経過したことを音および画面表示によってユーザーに知らせ、タイマー終了を見逃さないようにする                     |
| 表示機能     | タイマーの現在の状態や残り時間を分かりやすい形式で画面に表示し、ユーザーが経過状況を直感的に把握できるようにする                   |
| エラー処理   | 開始できない設定状態でタイマーが動作することを防ぎ、エラー内容を画面に表示することで意図しない動作や誤った操作を防止する            |

#### 2. アラーム

| 機能         | 目的        |
| ------------ | ---------- |
| アラーム追加 | ユーザーがタスク終了までの時間を設定できるようにする         |
| アラーム記録 | 過剰なアラーム設定数を制限し、有効的に本機能を使用する       |
| アラーム切替 | アラームを使用するかどうかをユーザーの状況に応じて対応する    |
| アラーム削除 | 不要なアラームを削除する事で、効率的に本機能を使用する       |
| アラーム鳴動 | 音で鳴らす事で聴覚情報によるユーザーのタスク管理・支援をする  |
| アラーム鳴止 | 音を止めるタイミングを決める事でタスクの良い切替を促す       |

#### 3. ストップウォッチ

| 機能 | 目的 |
|-----|-----|
| 計測開始・停止（スタート／一時停止） | 計測を開始・一時停止することで、必要な時間区間の正確な計測ができるようにする   |
| ラップ記録    | 任意のタイミングでラップを記録し、途中経過を把握、比較、管理できるようにする                     |
| リセット      | 計測結果やラップ履歴をクリアし、初期状態に戻すことで、新しい計測を準備できるようにする            |
| 経過時間表示  | 計測中の時間をリアルタイムに分かりやすく表示し、ユーザーへ即時の情報提供を行う                    |
| ラップ履歴表示 | 記録したラップタイムを一覧表示し、各ラップの時間や合計時間が分かるようにする                     |
...
- 　**5つ**までタイマーを設定できる(タブを閉じると削除される)

#### 2. 開始機能

-  時間を設定した時点で、設定した時間を画面に表示する。開始ボタン押下後、**1秒**ごとに残り時間を減少させる。
...
-  カウントダウン中または一時停止中にリセットボタンを押下した場合、カウントダウンを中断し、残り時間を設定時間に戻す
...
-  機内モード時(Wi-FiやBluetoothのoff時)は、音は鳴る
...
- 追加ボタン押下での「追加画面」の表示時刻は、その時の現時刻を取得する
...
- **5つ**までアラームを記憶できる（記憶したアラームはタブを閉じても保存される）⇒ **ローカルストレージ**を利用

- 保存データがない場合は[初期状態](#52-初期状態)を表示する
...
- ローカルストレージのデータ不整合時はデータを初期化する

- アプリ起動時に、過去時刻の未処理アラームが存在する場合、通知を表示する

- 最大鳴動時間は設定していないのでユーザーがアラームを停止する
...
- 計測開始と同時にラップボタンが押下できるようにする

##### 2.　計測停止(一時停止)

- 計測中にスタート／一時停止ボタンを押下すると計測を一時停止する

- 一時停止中、ラップボタンは押下できないようにする

- 一時停止中、リセットボタンは押下できるようにする
...
- 5つまで**ラップ**部分を設定できる(タブを閉じると削除される)
...
- 本アプリはシンプルな機能のため、致命的なエラーは想定されないが、以下の例外的動作を考慮する
  ...
  - 【多重押下制御】  
    スタート／一時停止ボタン、ラップボタンは連続押下を防止する

  - 【最大計測時間超過】  
    計測時間が99時間59分59秒99を超えた場合、計測を自動停止し、ユーザーに通知する

  - 【非活性ボタンの操作】  
    無効化状態のボタンを押下しても、何も反応しない
...
- 画面最小化時またはバックグラウンド動作時における音声再生は、ブラウザおよびOSの制御に依存する。
  上記状態において音が鳴る場合があるが、その動作を保証するものではない
...
- 運用・保守
  - Google Chrome 最新版および Microsoft Edge 最新版で動作確認をする
...
- ユーザビリティ  
  - ボタンの状態（有効化/無効化）は視覚的に明確に区別可能とする
